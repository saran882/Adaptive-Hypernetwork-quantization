...
# --- Configuration ---
# Device: "cuda" for GPU, "cpu" for CPU. On CPU, generation is much slower but needs no GPU.
device = "cpu"
...
# On CPU we use float32 (better supported); on GPU float16 saves VRAM.
...
pipe = StableDiffusionXLPipeline.from_pretrained(
    ...
    torch_dtype=dtype,
    variant="fp16" if device == "cuda" else None  # fp16 variant for GPU; default (fp32) for CPU.
).to(device)
